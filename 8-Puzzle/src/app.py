...
class PuzzlePage(tk.Frame):
    def __init__(self, parent, controller, *args, **kwargs):
        ...
        self.current_board_state = tuple(range(6))
        self.goal_board_state = tuple(range(6))
        self.saved_board_state = tuple(range(6))
        
        self.tile_images = [ImageTk.PhotoImage(Image.open(f'src/assets/images/tile_{n}.png')) for n in range(6)]
    ...
    def populate_board(self, state, delay_time=0):
        if max(state) >= len(self.tile_images):
            print(f"Error: state contains value {max(state)}, but only {len(self.tile_images)} images are loaded.")
            return
        ...
        if path_to_goal:
            ...
            for action in path_to_goal:
                if self.is_stopped:
                    ...
                    self.transform_state(action, delay_time=0.005)
            else:
    ...
    def load_image(self):
        ...
        if not image_path or not os.path.exists(image_path):
            print("Image Error")
            image_path = "img-test\\RedHat.png"
        image = Image.open(image_path)

        piece_size = image.size[0] // 3
        piece_height = image.size[1] // 2
        ...
        for i in range(2):
            for j in range(3):
                if i == 0 and j == 0:
                    ...
                    piece_path = f'src/assets/images/tile_{2*i+j}.png'
